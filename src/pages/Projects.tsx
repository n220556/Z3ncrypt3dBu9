...
export default function Projects() {
  return <div className="space-y-12 bg-gray-950">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {categories.map(category => <Button key={category} variant="outline" size="sm" className="hover:bg-primary/10 hover:text-primary hover:border-primary/20">
            {category}
          </Button>)}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        return <Card key={project.id} className="group overflow-hidden border-border/50 hover:border-primary/20 transition-all duration-300 hover:shadow-elegant">
              ...
              <div className="relative h-48 bg-gradient-subtle p-8 flex items-center justify-center">
                ...
                <div className={`absolute top-4 right-4 px-2 py-1 rounded-full text-xs font-medium ${project.status === 'Completed' ? 'bg-green-500/10 text-green-500' : project.status === 'In Progress' ? 'bg-blue-500/10 text-blue-500' : 'bg-orange-500/10 text-orange-500'}`}>
                  {project.status}
                </div>
              </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.technologies.map(tech => <span key={tech} className="px-2 py-1 bg-secondary/10 text-secondary text-xs rounded-full border border-secondary/20">
                      {tech}
                    </span>)}
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  {project.liveUrl && <Button asChild size="sm" className="flex-1">
